	...
	if (_)
		if (arguments.length == 2) {
			if (_.value !== undefined)
				_.value = $;
			else if (_.innerText !== undefined)
				_.innerText = $;
			else if (_.innerHTML !== undefined)
				_.innerHTML = $
...
String.prototype.startWith = function(_, $) {
	return $ ? new RegExp(_ + "*", "i").test(this)
			: this.substr(0, _.length) == _
};
String.prototype.endWith = function(_, $) {
	var B = this.length, A = _.length;
	if (A > B)
		return false;
	if ($)
		return new RegExp(_ + "$", "i").test(this);
	else
		return (A == 0 || this.substr(B - A, A) == _)
};
String.prototype.remove = function(_, A) {
	...
	if (_ > 0)
		$ = this.substring(0, _);
	if (_ + A < this.length)
		$ += this.substring(_ + A, this.length);
	return $
};
...
String.prototype.toDate = function() {
	...
	var F = parseInt(B[0], 10), C = parseInt(B[1], 10) - 1, _ = parseInt(B[2],
			10), E = B[3] ? parseInt(B[3], 10) : 0, D = B[4] ? parseInt(B[4],
			10) : 0, A = B[5] ? parseInt(B[5], 10) : 0, $ = new Date(0);
	$.setFullYear(F);
	$.setMonth(C);
	$.setDate(_);
	$.setHours(E);
...
(function() {
	var A = Object.prototype.toString, _ = function($) {
		if (Z.isArray($) || $.callee)
			return true;
		if (/NodeList|HTMLCollection/.test(A.call($)))
			return true;
	...
	Z.apply(Z, {
		...
		each : function(D, B, $) {
			if (Z.isEmpty(D, true))
				return;
			if (!_(D) || Z.isPrimitive(D))
				D = [ D ];
			for ( var C = 0, A = D.length; C < A; C++)
				if (B.call($ || D[C], D[C], C, D) === false)
					return C
		},
		encodeURI : function($) {
		...
		urlEncode : function(A, _) {
			var B, D = [], C, $ = encodeURIComponent;
			for (C in A) {
				B = !Z.isDefined(A[C]);
				Z.each(B ? C : A[C], function(_, A) {
					D.push("&", $(C), "=", (_ != C || !B) ? $(_) : "")
				})
			}
			if (!_) {
		...
		urlDecode : function(C, B) {
			var E = {}, _ = C.split("&"), $ = decodeURIComponent, A, D;
			Z.each(_, function(_) {
				_ = _.split("=");
				A = $(_[0]);
				D = $(_[1]);
...
(function() {
	...
	($ = _.match(/msie ([\d.]+)/)) ? Z.isIE = $[1] : false;
	($ = _.match(/firefox\/([\d.]+)/)) ? Z.isFireFox = $[1] : false;
	($ = _.match(/chrome\/([\d.]+)/)) ? Z.isChrome = $[1] : false;
	($ = _.match(/opera.([\d.]+)/)) ? Z.isOpera = $[1] : false;
	($ = _.match(/version\/([\d.]+).*safari/)) ? Z.isSafari = $[1] : false
})();
Z.ns("Z.String");
Z.String.format = function($) {
	var A = arguments, _ = new Array();
	for ( var B = 1; B < A.length; B++)
		_.push(A[B]);
	return $.format(_)
};
Z.String.leftByte = function($, B, A) {
	if (!$ || !B || B >= String($).getByteLen())
		return $;
	if (A) {
		var _ = B - 3;
		while (C($, _).getByteLen() > _)
			_--;
		return C($, _) + "..."
	} else {
		while (C($, B).getByteLen() > B)
			B--;
		return C($, B)
	}
	function C($, _) {
		return $.replace(/([\u0391-\uffe5])/ig, "$1a").substring(0, _).replace(
				/([\u0391-\uffe5])a/ig, "$1")
	}
};
Z.ns("Z.Date");
...
Z.Date.fmt = function(F, B) {
	...
	var H = String(F.getFullYear()), C = String(F.getMonth() + 1), $ = String(F
			.getDate()), E = String(F.getHours()), G = F.getHours(), A;
	if (G <= 6)
		A = "\u51cc\u6668";
	else if (G <= 11)
		A = "\u4e0a\u5348";
	...
	if (G > 12)
		G = G - 12;
	G = String(G);
	var D = String(F.getMinutes()), _ = String(F.getSeconds());
	B = B.replace(/y{1,4}/ig, function(_, $) {
		return H.right(_.length)
	});
	B = B.replace(/M{1,2}/g, function(_, $) {
		return _.length == 1 ? C : C.leftPad(2, "0")
	});
	B = B.replace(/d{1,2}/ig, function(A, _) {
		return A.length == 1 ? $ : $.leftPad(2, "0")
	});
	B = B.replace(/H{1,2}/g, function(_, $) {
		return _.length == 1 ? E : E.leftPad(2, "0")
	});
	B = B.replace(/h{1,2}/g, function(_, $) {
		return _.length == 1 ? G : G.leftPad(2, "0")
	});
	B = B.replace(/m{1,2}/g, function($) {
		return $.length == 1 ? D : D.leftPad(2, "0")
	});
	B = B.replace(/s{1,2}/ig, function($) {
...
Z.addEvent = function(_, A, C, B) {
	...
	if (_) {
		...
		if (window.attachEvent)
			...
			_.addEventListener(A, $, false)
	} else
		alert("\u53c2\u6570target\u65e0\u6548,\u8bf7\u68c0\u67e5!")
};
Z.removeEvent = function(A, $, _) {
	if (!A || !$ || $.trim().length < 4 || !Z.isFunction(_))
		return;
	$ = $.toLowerCase().trim();
	if ($.startWith("on"))
		$ = $.right($.length - 2);
...
//M表示容器id，比如某个form或div的id
//_表示默认对象，如果没有，则新建一个对象
//J表示是否trim（）字符串
//E表示剔除一些属性
Z.getUI2Params = function(M, _, J, E) {
	try {
		var B = (M ? $E(M) : document), C = (_ ? _ : {});
		if (!B) {
			alert("containerId=" + M
					+ "\u7684\u5bb9\u5668\u5bf9\u8c61\u672a\u627e\u5230!");
			return C
		...
		alert("function getUI2Params(...)\u51fd\u6570\u51fa\u9519\uff1a"
				+ A.name + "->" + A.message)
	}
};
//直接传入id数组,表示要得到哪些dom值作为参数
	...
	for ( var E = 0; E < _.length; E++) {
		...
		if (C && C.type != "radio") {
			if ((C.type == "checkbox" && C.checked)
				...
				A[$] = C.type == "checkbox" ? C.checked : C.value.trim()
		} else {
			var B = document.getElementsByName($);
			for ( var D = 0; D < B.length; D++)
				if (B[D].checked && B[D].value) {
					A[$] = B[D].value;
					break
				}
		}
...
Z.openWindow = function() {
	var C = null, _ = arguments.length;
	if (_ == 0)
		return false;
	else
		C = arguments[0];
	var B = screen.availWidth - 10, A = screen.availHeight, D = 0, $ = 0;
	...
	window
			.open(
					C,
					"",
					"menubar=no,toolbar=no,location=no,directories=no,status=no,scrollbars=yes,width="
							+ B
							+ ",height="
							+ A
							+ ",left="
...
Z.Url.getParam = function($) {
	var D, B, A, _, C = window.location.href, G = C.indexOf("?"), F = C
			.substring(G + 1);
	if (!Z.isDefined($))
		return Z.urlDecode(F);
	if (G <= 0)
		return null;
	if (C.indexOf("&") > 0) {
		D = F.split("&");
		for ( var E = 0; E < D.length; E++) {
			...
			if (B.toUpperCase() == $.toUpperCase())
				return Z.decodeURI(A)
		}
	} else {
		_ = F;
		G = _.indexOf("=");
...
Z.Url.go = function($) {
	document.location.href = $ + ($.indexOf("?") == -1 ? "?" : "&") + "random="
			+ new Date().getTime()
};
...
if (Z.isFireFox)
	HTMLElement.prototype.__defineGetter__("children", function() {
		...
		for ( var A = 0; A < this.childNodes.length; A++)
			if (this.childNodes[A].nodeType == 1) {
				$[_] = this.childNodes[A];
				_++
			}
		$.length = _;
...
if (Z.isFireFox) {
	HTMLElement.prototype.__defineGetter__("outerHTML", function() {
		var _ = this.attributes, $ = "<" + this.tagName, A = 0;
		for (; A < _.length; A++)
			if (_[A].specified)
				$ += " " + _[A].name + "=\"" + _[A].value + "\"";
	...
	HTMLElement.prototype.__defineSetter__("outerHTML", function(_) {
...
Z.Ajax.request = function(A) {
	function E() {
		var _, A = [ "MSXML2.XMLHTTP.3.0", "MSXML2.XMLHTTP",
				"Microsoft.XMLHTTP" ];
		try {
	...
	var D = A.url, C = A.params, B = A.method;
	if (!Z.isDefined(A.method))
		B = Z.isDefined(C) ? "POST" : "GET";
	if (C)
		if (B == "GET") {
...
Z.getFileSize = function($, _) {
	...
	if ($ < 1024)
		...
		return A($ / (1024) - 0.005, _) + "KB";
	else if ($ < 1073741824)
		return A($ / (1048576) - 0.005, _) + "MB";
	else
		return A($ / (1073741824) - 0.005, _) + "GB";
	function A(_, A) {
		var $ = Math.round(_ * Math.pow(10, A)) / Math.pow(10, A), B = String($)
				.split(".");
		if (B[0].length > 3)
			return B[0];
		else
...
Z.getEvent = function() {
	...
	while (_ != null) {
		var $ = _.arguments[0];
		if ($)
			if (($.constructor == Event || $.constructor == MouseEvent)
					|| (typeof ($) == "object" && $.preventDefault && $.stopPropagation))
				return $;
		_ = _.caller
	}
	return null
...
Z.preventDefault = function($) {
	$ = $ ? $ : Z.getEvent();
	if (Z.isFunction($.preventDefault))
		$.preventDefault();
	else
		$.returnValue = false
};
...
Z.disp = function(A, $, _) {
	...
	function B($, _, A) {
		...
		if (Z.isString(A))
			A = A.toLowerCase().replaceAll(";", "").replaceAll(" ", "");
		else
			A = "";
		if (Z.isBoolean(_))
			$.style.display = _ ? A : "none";
		else if ($.style.display.toLowerCase() == A)
			$.style.display = "none";
		else
			$.style.display = A
	}
...
Z.openMaxWindow = function(D, A) {
	var $ = screen.availWidth, E = screen.availHeight, B = 0, _ = 0;
	if (!Z.isDefined(A) || A == null)
		A = "maxWin" + Z.getTime();
	var C = window
			.open(
					D,
	...
	if (Z.isIE)
		C.resizeTo(screen.availWidth + 8, screen.availHeight + 8);
	else {
...
Z.importJS = function(B, $) {
	...
	function A(G, B) {
		...
		if (typeof _ == "function") {
			var F = document.getElementsByTagName("head")[0], E = F.childNodes;
			for ( var I = 0; I < E.length; I++) {
				var $ = E[I], C = $.nodeName.toUpperCase();
				if (C == "SCRIPT" && D($, "src") == G || C == "LINK"
						&& D($, "href") == G) {
					alert("\u91cd\u590d\u52a0\u8f7d" + G);
					return
				}
		...
		function D(B, $) {
			try {
				var A = B.attributes.getNamedItem, C = B.attributes
						.getNamedItem($);
...
Z.importCSS = function(B, $) {
	...
	function A(G, A) {
		...
		if (typeof _ == "function") {
			var F = document.getElementsByTagName("head")[0], E = F.childNodes;
			for ( var I = 0; I < E.length; I++) {
				var $ = E[I], B = $.nodeName.toUpperCase();
				if (B == "SCRIPT" && C($, "src") == G || B == "LINK"
						&& C($, "href") == G) {
					alert("\u91cd\u590d\u52a0\u8f7d" + G);
					return
				}
...
Z.htmlGB2132 = function($) {
	...
	for (i = 0; i < $.length; i++) {
		var A = $.charCodeAt(i);
		if (A <= 255 || (A >= 13312 && A <= 19893)
				|| (A >= 19968 && A <= 40869) || (A >= 40870 && A <= 40891)
				|| (A >= 63744 && A <= 64045) || (A >= 64048 && A <= 64106)
				|| (A >= 64112 && A <= 64217) || (A >= 131072 && A <= 173782)
				|| (A >= 194560 && A <= 195101) || (A >= 65280 && A <= 65519)
				|| (A >= 12288 && A <= 12351) || (A >= 65040 && A <= 65055)
				|| (A >= 65072 && A <= 65103)
				|| (A == 8212 || A == 8220 || A == 8221 || A == 8230))
			_[i] = $.charAt(i);
		else
			_[i] = "&#" + $.charCodeAt(i) + ";"
	}
...
Z.htmlDecodeAll = function($) {
	return $.replace(/&#(x)?([^&]{1,5});?/g, function(A, $, _) {
...
Z.encodeSCRIPT = function($) {
	...
	return $.replace(_, function($, A, _) {
		return $.replace(/<(.*?)>/g, "&lt;$1&gt;") + A
				+ _.replace(/<(.*?)>/g, "&lt;$1&gt;")
	})
};
...
Z.Unicode = {
	on : function($) {
		var _ = [];
		for (i = 0; i < $.length; i++)
			_[i] = ("00" + $.charCodeAt(i).toString(16)).slice(-4);
		return "\\u" + _.join("\\u")
	},
	un : function($) {
...
Z.limitImg = function(B, I, J, D, _, $, F) {
	...
	if (!Z.isDefined(F))
		F = true;
	var H, G, A = 100, E = new Image(), C = function() {
...
Z.removeEmpty = function(_) {
	...
	var $ = [];
	for ( var A in _)
		if (_.hasOwnProperty(A) && Z.isEmpty(_[A]))
			$.push(A);
	Z.each($, function($) {
		delete _[$]
	});
...
Z.ajax = function(C, B, $) {
	var A = !Z.isFunction($), _;
	if (C.indexOf(".ajax?action") == -1)
		C = "1.ajax?action=" + C;
	Z.Ajax.request({
		url : C,
		params : B,
...
Z.decode = function($) {
	if ($ === undefined || $ === null || $.trim() === "")
		return null;
	else
		return Z.isObject($) ? $ : window.eval("(" + $ + ")")
};
Z.JSON = new (function() {
	var D = !!{}.hasOwnProperty, A = function() {
		var $ = null;
		return function() {
			if ($ === null)
				$ = Z.USE_NATIVE_JSON && window.JSON
		...
		if (!Z.isDefined(F) || F === null)
			return "null";
		else if (Z.isArray(F))
			return E(F);
		else if (Z.isDate(F))
			return Z.JSON.encodeDate(F);
		else if (Z.isString(F))
			return _(F);
		else if (typeof F == "number")
			return isFinite(F) ? String(F) : "null";
		else if (Z.isBoolean(F))
			return String(F);
		else {
			var C = [ "{" ], A, G, $;
			for (G in F)
				if (!F.getElementsByTagName)
					if (!D || F.hasOwnProperty(G)) {
						$ = F[G];
						switch (typeof $) {
						case "undefined":
						case "function":
...
Z.showMsg = function(E, C) {
	...
	var _ = "showmsg" + parseInt(Math.random() * 10000), B = ("<div id=\"{0}\" class=\"z-showmsg\" style=\"display:none\"><div class=\"wrap\">"
			+ E + "</div></div>").format(_), A = $(window), D = $(B).prependTo(
			"body");
	C = Z.isObject(C) ? C : {};
	C.delay = Z.isDefined(C.delay) ? C.delay : 2000;
	var F = 0;
	if (Z.isIE && Z.isIE <= 6)
		F = A.scrollTop() + ((A.height() - D.outerHeight()) / 2);
	else
		F = Math.round((A.height() - D.height()) / 2);
	D.css("z-index", Z.showMsg.zIndex).css("top", F).css("left",
			(A.width() - D.width()) / 2).fadeIn("fast").delay(C.delay).fadeOut(
			"slow", function() {
				$(this).remove();
...
Z.LocalStore = {
	get : function($) {
		$ = String($);
		return !window.localStorage ? Z.UserData.getItem($) : localStorage
				.getItem($)
	},
	set : function(_, $) {
		_ = String(_);
		!window.localStorage ? Z.UserData.setItem(_, $) : localStorage.setItem(
				_, $)
	},
	remove : function($) {
		$ = String($);
		!window.localStorage ? Z.UserData.remove($) : localStorage.removeItem($) } }